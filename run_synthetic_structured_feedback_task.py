...
METHODS = ["initial", "bio_prior_only", "feedback_only", "full_risk_gate"]
...
def make_dataset(seed, n=480, m_pairs=12, d=10):
    rng = np.random.default_rng(seed)
    z = rng.normal(size=(n, d))
    w = rng.normal(scale=0.75, size=(m_pairs, d))
    side_shift = rng.normal(scale=0.25, size=(2, m_pairs))
    logits_left = z @ w.T + side_shift[0]
    logits_right = z @ w.T + side_shift[1] + 0.35 * (z[:, :1] @ rng.normal(size=(1, m_pairs)))
    logits = np.hstack([logits_left, logits_right])
    patient_severity = 0.35 * z[:, 0:1] - 0.25 * z[:, 1:2]
    logits += patient_severity
    probs = sigmoid(logits)
    y = (rng.random(probs.shape) < probs).astype(int)
    x = np.hstack([z, z[:, :4] ** 2, rng.normal(scale=0.4, size=(n, 6))])
    x += rng.normal(scale=0.15, size=x.shape)
    ids = np.arange(n)
    return ids, x.astype(np.float32), y.astype(int)
...
def predict_probabilities(x_train, y_train, x_test):
    ...
    for i in range(len(x_test)):
        ...
        p21 = weighted_knn(dist[i], y_train, 21)
        rows.append(0.50 * p7 + 0.35 * p21 + 0.15 * prior)
    return np.clip(np.vstack(rows), 0.02, 0.98)
...
def score(y_true, y_pred):
    ...
    return 0.5 * (item_acc + 1.0 - rmse / float(m))
...
def metric_row(y_true, y_pred):
    return {
        ...
        "total_rmse": float(np.sqrt(np.mean((y_true.sum(axis=1) - y_pred.sum(axis=1)) ** 2))),
    }
...
def run_bio_prior_only(initial_pred, prob, budget):
...
def run_split(seed, args):
    ids, x, y = make_dataset(args.data_seed)
    rng = np.random.default_rng(seed)
    order = ids.copy()
    rng.shuffle(order)
    n_test = int(round(len(ids) * args.test_fraction))
    test_ids = set(map(int, order[:n_test]))
    train_mask = np.asarray([int(i) not in test_ids for i in ids], dtype=bool)
    x_train, x_test = standardize(x[train_mask], x[~train_mask])
    ...
    for method in METHODS:
        ...
        elif method == "bio_prior_only":
            pred, trace = run_bio_prior_only(initial_pred, prob, args.budget)
        else:
            method_rng = np.random.default_rng(seed + args.budget * 101 + METHODS.index(method) * 1009)
...
def main():
    ...
    parser.add_argument("--data-seed", type=int, default=20260600)
    ...
    parser.add_argument("--out-prefix", default="synthetic_structured_feedback_v1")
    args = parser.parse_args()

    rows = []
    for i in range(args.splits):
        rows.extend(run_split(args.seed + i * 17, args))
    ...
    json_path.write_text(json.dumps({"config": vars(args), "summary": summary}, indent=2), encoding="utf-8")
    lines = [
        "# Synthetic Structured Multi-Label Feedback Task",
        "",
